...
- **SUB-12 / PCI-DSS 10.2 rejected transfers:** `payments/src/transfer.ts` writes rejected attempts (`same_account`, `missing_fields`, `invalid_amount`) to the audit log with `status: rejected` and a `reason`.
